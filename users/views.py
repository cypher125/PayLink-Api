...
class VTPassPurchaseView(APIView):
    ...
    def post(self, request):
        ...
        if request.user.vtpass_balance < amount_decimal:
            return Response({
                'success': False,
        ...
        if (response.get('code') == 'success' or 
            ...
            request.user.vtpass_balance = float(request.user.vtpass_balance) - amount_decimal
            request.user.save()
        else:
...
class FundWalletView(APIView):
    ...
    def post(self, request):
        ...
        if payment_method == 'bank_transfer':
            ...
            user = request.user
            user.vtpass_balance = float(user.vtpass_balance) + amount
            user.save()
        else:
        ...
        return Response({
            ...
            'updated_balance': float(request.user.vtpass_balance)
        }, status=status.HTTP_200_OK)
